...
// args::ValueFlag<std::string> calibration_dir(group_optional, "Calibration file XML", "XML file containing camera matrix and distortion coefficients", {'x'});
args::ValueFlag<int> blender_bands(group_optional, "", "number of bands for multi-band blender (available for graph-cut option only).", {'n'});
args::Flag use_grid(group_optional, "grid", "Filter key points based on grid distribution. The grid is fixed at 10x10 cells.", {"grid"});
args::Flag final_scb(group_optional, "SCB", "Apply Simple-Color-Balance algorithm to enhance final image.", {"scb"});
args::Flag euclidean_mode(group_optional, "", "Euclidean mode, apply only euclidean transformations (rotation and translation).", {'e'});
args::ValueFlag<int> euclidean_local_correction(group_optional, "level", "Euclidean local correction Level (0[ANY], 1[SOFT] or 2[HARD]), approximate projective transformation to best euclidean.", {'l'});
args::Flag graph_cut(group_optional, "", "Graph-cut seam finder --pixel level-- (slower).", {'g'});
args::Flag output(group_optional, "print", "Show final blended images.", {'o'});
args::Flag color_c(group_optional, "color_correction", "apply color correction", {'c'});